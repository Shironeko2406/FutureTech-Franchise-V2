...
const ShowReportModal = ({ visible, onClose, taskId, taskType }) => {
  ...
  const handleSaveReport = async () => {
    setLoading(true);
    try {
      console.log("handleSaveReport", uploadedFileURL);
      const values = await form.validateFields();
      const formattedValues = {
        ...values,
        reportImageURL: uploadedFileURL || (values.reportImageURL && values.reportImageURL[0] ? values.reportImageURL[0].url : null),
      };
